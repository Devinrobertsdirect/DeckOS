...
### OpenClaw — AI security and permission layer ✅ Complete (Month 2)

OpenClaw is DeckOS's completed AI containment model. It sits between the AI inference tier and the hardware arbitration layer and defines exactly what the AI is and is not allowed to do at runtime.

**Implemented components:**

| Component | Status | Description |
|-----------|--------|-------------|
| Command schema validator | ✅ Done | Rejects any AI output that doesn't conform to the registered command schema for the active module — the AI cannot invent commands at runtime |
| Intent scoring classifier | ✅ Done | Scores write operations against the user's stated session goal before execution; low-scoring commands queue for manual confirmation |
| Audit log | ✅ Done | Append-only SQLite table recording every AI-generated command (raw output, parsed command, timestamp, execution status) |
| Permission scoping system | ✅ Done | User declares a scope at session start: `READ`, `EXECUTE`, or `ADMIN` — each is a whitelist of arbitration handler IDs; calls outside the whitelist are rejected and logged |

**Session scopes:**
```
READ    — AI can query state and display information only
EXECUTE — AI can run pre-approved module commands
ADMIN   — AI can write configs and call arbitration handlers
```

The AI is never told why a command was rejected — it receives a generic "action unavailable" response to prevent the model from learning permission boundaries through trial and error.
...
- **OpenClaw Security Layer** ✅ — Command schema validation, intent scoring, audit log, and session-scoped permission whitelists between the AI and hardware
...
- **Projector Output Mode** ✅ — HDMI dual-display with mirror mode; display profile flag disables frame animations for clean projected output
- **GPIO IoT Hub** 🔧 In Progress — Physical devices (sensors, relays, LEDs) registered as first-class arbitration actors; AI can read INPUT devices in any scope, OUTPUT devices require EXECUTE permission and user confirmation
...
| `physical_devices` | device_id, pin, type (INPUT/OUTPUT), handler_id, last_value, last_read_at, dirty | GPIO and physical IoT device registry; `dirty` flag marks values older than 30 s so the AI tier always sees a freshness indicator rather than a live poll |
...
**Sensors**
- DHT22 / DHT11 — temperature + humidity (Arduino/ESP MQTT publisher) · **GPIO INPUT, wired and active on Pi**
- BME280 / BME680 — temperature, humidity, pressure, air quality
- DS18B20 — waterproof temperature probe
- PIR HC-SR501 — passive infrared motion sensor · **GPIO INPUT, wired and active on Pi**
...
**Output devices (GPIO OUTPUT, wired and active on Pi)**
- LED indicator strip — controlled via arbitration layer (EXECUTE scope required); triggers on PIR motion event
- Piezo buzzer — alert output; EXECUTE scope required; used for audio feedback on AI confirmation prompts
...
**Display & Projection**
- 7" IPS touchscreen — primary display (HDMI + USB touch, mounted inside chassis lid)
- HDMI projector — secondary output via Pi dual-display; mirror mode built into the display module; `projector` display profile disables frame animations to eliminate latency artifacts at 40 ms HDMI delay

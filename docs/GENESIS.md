...
> A developer's guide to what happens the *very first* time someone opens Atlas, before
...
Both gates are checked on load. If both are already satisfied, Atlas boots straight into
the app in the last-used `atlas_ui_mode`.

## Act 1 — Setup wizard

Runs while `isSetupDone()` is `false`. It collects three things and nothing else — the
goal is "talking to Atlas in under a minute," not a settings marathon.
...
3. **Voice engine.** `setVoiceEngine("browser" | "server")`. `"browser"` is the
   ...
   key is saved and gives Atlas a real, waveform-driven voice. Stored in
   `atlas_voice_engine`.
...
After the intro, Atlas lands in the app in one of two UI modes (`useUiMode()` /
...
| `atlas_user_name` | `setUserName()` | What Atlas calls the user; spoken in the intro. |
